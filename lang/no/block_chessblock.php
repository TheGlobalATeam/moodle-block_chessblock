<?php
$string['pluginname'] = 'Sjakk Blokk';
$string['chessblock'] = 'Sjakk Blokk';
$string['blockstring'] = 'Sjakk Blockk';
$string['chessblock:addinstance'] = 'Legg til ny simpel Sjakk Blokk';
$string['chessblock:myaddinstance'] = 'Legg til ny simpel Sjakk Blokk til min Moodle side';

// Static html strings
$string['blocktitle'] = 'Sjakk';
$string['newgamebutton'] = 'Start nytt spill mot datamaskinen';
$string['gamestatus'] = 'Status';
$string['userid'] = 'Bruker ID';

// Dynamic js strings
$string['white'] = 'Hvit';
$string['black'] = 'Svart';
$string['tomove'] = 'i trekket';
$string['gameover'] = 'Spillet er over';
$string['isincheck'] = 'er i sjakk';
$string['isincheckmate'] = 'er i sjakk matt';
$string['drawnposition'] = 'Uavgjort';
